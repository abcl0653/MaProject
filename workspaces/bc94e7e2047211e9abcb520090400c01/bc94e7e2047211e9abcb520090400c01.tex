...
\question 如图，在$\triangle ABC$中，$\angle ACB=90\degree$，$\angle A=30 \degree,~D$是边$AC$上不与点$A$、$C$重合的任意一点，$DE\bot AB$，垂足为点$E$，$M$是$BD$的中点.
\begin{parts}
\part 求证：$CM=EM$~;
\part 如果~$BC=\sqrt{3}$,设~$AD=x,~CM=y$，求$y$与$x$的函数解析式，并写出定义域；
\part 当点$D$在线段$AC$上移动时，$\angle MCE$~的大小是否发生变化？若不变，求出~$\angle MCE$~的大小；如果发生变化，说明如何变化.
\end{parts}
